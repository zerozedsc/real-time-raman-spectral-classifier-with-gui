...
from PySide6.QtWidgets import (
    ...
    QListWidget, QListWidgetItem, QMessageBox, QTabWidget, QTextEdit
)
from PySide6.QtCore import Qt, Signal, QUrl

from functions.data_loader import load_data_from_path, load_metadata_from_json, plot_spectra
from utils import LOCALIZE, PROJECT_MANAGER, CONFIGS, RAMAN_DATA
from components.matplotlib_widget import MatplotlibWidget

class DragDropLabel(QLabel):
    """A custom QLabel that accepts file/folder drops."""
...
class DataPackagePage(QWidget):
    """A page for loading, viewing, and managing multiple spectral datasets within a project."""
    ...
    def _create_left_panel(self, parent_layout):
        ...
        # Importer Group
        importer_group = QGroupBox(LOCALIZE("DATA_PACKAGE_PAGE.importer_title"))
        importer_layout = QGridLayout(importer_group)
        importer_layout.setSpacing(10)
        importer_layout.addWidget(QLabel(f"<b>{LOCALIZE('DATA_PACKAGE_PAGE.dataset_name_label')}:</b>"), 0, 0, 1, 2)
        self.dataset_name_edit = QLineEdit()
        self.dataset_name_edit.setPlaceholderText(LOCALIZE("DATA_PACKAGE_PAGE.dataset_name_placeholder"))
        importer_layout.addWidget(self.dataset_name_edit, 1, 0, 1, 2)
        self.data_drop_label = DragDropLabel(LOCALIZE("DATA_PACKAGE_PAGE.drag_drop_data_prompt"))
        importer_layout.addWidget(self.data_drop_label, 2, 0, 1, 2)
        self.data_path_edit = QLineEdit()
        self.data_path_edit.setReadOnly(True)
        importer_layout.addWidget(self.data_path_edit, 3, 0)
        browse_data_btn = QPushButton(LOCALIZE("DATA_PACKAGE_PAGE.browse_data_button"))
        importer_layout.addWidget(browse_data_btn, 3, 1)
        self.meta_drop_label = DragDropLabel(LOCALIZE("DATA_PACKAGE_PAGE.drag_drop_meta_prompt"))
        importer_layout.addWidget(self.meta_drop_label, 4, 0, 1, 2)
        self.meta_path_edit = QLineEdit()
        self.meta_path_edit.setReadOnly(True)
        importer_layout.addWidget(self.meta_path_edit, 5, 0)
        browse_meta_btn = QPushButton(LOCALIZE("DATA_PACKAGE_PAGE.browse_meta_button"))
        importer_layout.addWidget(browse_meta_btn, 5, 1)
        self.preview_button = QPushButton(LOCALIZE("DATA_PACKAGE_PAGE.preview_button"))
        importer_layout.addWidget(self.preview_button, 6, 0)
        ...
        importer_layout.addWidget(self.add_to_project_button, 6, 1)

        # Loaded Datasets Group
        ...
        self.remove_button = QPushButton(LOCALIZE("COMMON.remove"))
        actions_layout.addWidget(self.remove_button)
        actions_layout.addStretch()
        loaded_layout.addWidget(self.loaded_data_list)
        loaded_layout.addLayout(actions_layout)

        left_vbox.addWidget(importer_group)
        left_vbox.addWidget(loaded_group)
        left_vbox.addStretch()
        parent_layout.addLayout(left_vbox)

        # Connect signals
        self.data_drop_label.pathDropped.connect(self._set_data_path)
        self.meta_drop_label.pathDropped.connect(self.meta_path_edit.setText)
    ...
    def _create_right_panel(self, parent_layout):
        ...
        preview_layout.addWidget(self.plot_widget, 5)
        preview_layout.addWidget(self.info_label, 1)

        # Metadata Editor Group
        self.meta_editor_group = QGroupBox(LOCALIZE("DATA_PACKAGE_PAGE.metadata_editor_title"))
        self.meta_editor_group.setCheckable(True)
        ...
        editor_vbox.addWidget(self.meta_tabs)
        editor_vbox.addWidget(self.save_meta_button, 0, Qt.AlignmentFlag.AlignRight)

        right_vbox.addWidget(preview_group)
        right_vbox.addWidget(self.meta_editor_group)
    ...
    def _connect_signals(self):
        ...
        self.remove_button.clicked.connect(self.handle_remove_selected_dataset)
    ...
    def _set_data_path(self, path: str):
        ...
        if path:
            ...
            if not self.dataset_name_edit.text().strip():
                self.dataset_name_edit.setText(base_name.replace('_', ' ').replace('-', ' ').title())

    def load_project_data(self):
        current_selection = self.loaded_data_list.currentItem().text() if self.loaded_data_list.currentItem() else None
        self.loaded_data_list.clear()
        
        if not RAMAN_DATA:
            self.loaded_data_list.addItem(LOCALIZE("DATA_PACKAGE_PAGE.no_datasets_loaded"))
            self.loaded_data_list.setEnabled(False)
            self.remove_button.setEnabled(False)
            self.update_preview_display(None, {})
        else:
            self.loaded_data_list.addItems(RAMAN_DATA.keys())
            self.loaded_data_list.setEnabled(True)
            self.remove_button.setEnabled(True)
            # Try to re-select the previously selected item
            items = self.loaded_data_list.findItems(current_selection, Qt.MatchExactly)
            if items:
                self.loaded_data_list.setCurrentItem(items[0])
    ...
    def browse_for_data(self):
        dialog = QFileDialog(self, LOCALIZE("DATA_PACKAGE_PAGE.browse_data_dialog_title"))
        dialog.setFileMode(QFileDialog.FileMode.ExistingFiles)
        dialog.setOption(QFileDialog.Option.ShowDirsOnly, False)
        if dialog.exec():
            self._set_data_path(dialog.selectedFiles()[0])
    ...
    def handle_add_to_project(self):
        ...
        if not dataset_name:
            self.showNotification.emit(LOCALIZE("NOTIFICATIONS.dataset_name_missing"), "error")
            return
    ...
    def handle_remove_selected_dataset(self):
        current_item = self.loaded_data_list.currentItem()
        if not current_item:
            return
        name = current_item.text()
        
        reply = QMessageBox.question(self, LOCALIZE("DATA_PACKAGE_PAGE.remove_dataset_confirm_title"),
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if PROJECT_MANAGER.remove_dataframe_from_project(name):
                ...
                self.showNotification.emit(LOCALIZE("NOTIFICATIONS.dataset_remove_error", name=name), "error")
    ...
    def update_preview_display(self, df: pd.DataFrame, metadata: dict, is_preview: bool = True):
        ...
        if df is not None and not df.empty:
            info_text = (f"<b>{LOCALIZE('DATA_PACKAGE_PAGE.info_num_spectra')}:</b> {df.shape[1]} | "
        ...
        self.meta_editor_group.setChecked(bool(metadata) or is_preview)
        self.toggle_metadata_editing(self.meta_editor_group.isChecked(), read_only=not is_preview)
    ...
    def _get_metadata_from_editor(self) -> dict:
        ...
        for tab_key, fields in self.metadata_widgets.items():
            ...
            for field_key, widget in fields.items():
                metadata[tab_key][field_key] = widget.toPlainText() if isinstance(widget, QTextEdit) else widget.text()
        return metadata
    ...
    def save_metadata_as_json(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, LOCALIZE("DATA_PACKAGE_PAGE.save_meta_dialog_title"), os.path.join(data_dir, default_filename), "JSON Files (*.json)")
        if path:
            manual_meta = self._get_metadata_from_editor()
            try:
                with open(path, 'w', encoding='utf-8') as f:
                    json.dump(manual_meta, f, indent=4)
                self.showNotification.emit(LOCALIZE("NOTIFICATIONS.meta_save_success"), "success")
                self.meta_path_edit.setText(path)
                self.preview_metadata = manual_meta
            except Exception as e:
                self.showNotification.emit(LOCALIZE("NOTIFICATIONS.meta_save_error", error=e), "error")
