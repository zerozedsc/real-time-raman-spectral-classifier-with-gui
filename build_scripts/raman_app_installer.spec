...
"""
PyInstaller spec file for Raman Spectroscopy Analysis Application - NSIS Installer
Generated: October 21, 2025

This spec creates an executable that will be packaged by NSIS installer builder.
Same as portable version but with installer-specific output structure.

Build command:
    pyinstaller raman_app_installer.spec

Output location:
    build_installer/ (staging directory for NSIS)
"""
...
if '__file__' in globals():
    ...
    if candidate.name != 'raman_app_installer.spec':
        candidate = Path(os.getcwd()) / 'build_scripts' / 'raman_app_installer.spec'
    spec_path = candidate
...
# ============== BUNDLE ==============
# Creates build_installer/ directory for NSIS packaging
coll = COLLECT(
    exe,
    a.binaries,
    a.zipfiles,
    ...
    upx=True,
    upx_exclude=[],
    name='raman_app_installer_staging',
)
